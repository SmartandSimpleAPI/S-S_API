package fr.dojo59.api.compile.ItemAPI;
...
public class ItemBuilder extends ItemStack {
    ...
    public boolean isSimilar(ItemStack item) {
        if (item != null) {
            ...
            if (itMeta != null) {
                List<String> itLore = itMeta.getLore() != null ? itMeta.getLore() : new ArrayList<>();
                List<String> lore = meta.getLore() != null ? new ArrayList<>(meta.getLore()) : new ArrayList<>();
                lore.removeAll(itLore);

                String itDisplayName = itMeta.hasDisplayName() ? itMeta.getDisplayName() : "";
                String displayName = meta.hasDisplayName() ? meta.getDisplayName() : "";

                Integer itCustomModelData = itMeta.hasCustomModelData() ? itMeta.getCustomModelData() : -1000000000;
                Integer customModelData = meta.hasCustomModelData() ? meta.getCustomModelData() : -1000000000;

                Integer itMaxStackSize = itMeta.hasMaxStackSize() ? itMeta.getMaxStackSize() : null;
                Integer maxStackSize = meta.hasMaxStackSize() ? meta.getMaxStackSize() : null;

                Map<Enchantment, Integer> itEnchants = itMeta.hasEnchants() ? new HashMap<>(itMeta.getEnchants()) : null;
                Map<Enchantment, Integer> enchants = meta.hasEnchants() ? new HashMap<>(meta.getEnchants()) : null;

                return (itMeta.hasLore() == meta.hasLore()) &&
                        (itMeta.hasRarity() == meta.hasRarity()) &&
                        Objects.equals(itEnchants, enchants) &&
                        lore.isEmpty() &&
                        Objects.equals(itDisplayName, displayName) &&
                        Objects.equals(itCustomModelData, customModelData) &&
                        Objects.equals(itMaxStackSize, maxStackSize);
            }
        }
        return false;
    }
