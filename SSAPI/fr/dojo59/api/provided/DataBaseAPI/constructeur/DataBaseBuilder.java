...
public class DataBaseBuilder<T extends FieldTable> extends Cache<T> {
    ...
    private final MySQLConnectionFile sqlConnectionFile;
    private final MySQLTableFile sqlTableFile;
    ...
    public DataBaseBuilder(Class<T> table) throws SQLException {
        ...
        this.sqlConnectionFile = new MySQLConnectionFile();
        this.sqlTableFile = new MySQLTableFile(tableName, tableName);
        this.table = table;
    ...
    private Connection getConnection() throws SQLException {
        if (this.connection == null) {
            String idConnection = sqlTableFile.getString(".Connection.IdConnection");

            String url = sqlConnectionFile.getString("." + idConnection + ".URL");
            String user = sqlConnectionFile.getString("." + idConnection + ".USER");
            String password = sqlConnectionFile.getString("." + idConnection + ".PASSWORD");
    ...
    public MySQLTableFile getSqlTableFile() {
        return this.sqlTableFile;
    }

    public MySQLConnectionFile getSqlConnectionFile() {
        return this.sqlConnectionFile;
    }
    ...
    private void initializedDataBase() throws SQLException {
        ...
        if (this.getConnection() == null) {
            ...
            if (this.identifierType.equalsIgnoreCase("UUID")) {
                ...
                if (this.sqlTableFile.getStringList(".IdentifierList").size() > 1) {
                    Console.println(" ");
                    Console.log(SSAPI_Plugin.getINSTANCE().getLogger(), ConsoleColor.LIGHT_PURPLE + "Launch of the caching of " + ConsoleColor.AQUA + this.sqlTableFile.getStringList( ".IdentifierList").size() + ConsoleColor.LIGHT_PURPLE + " identifiers in " + ConsoleColor.AQUA + this.tableName);
                } else {
                    Console.println(" ");
                    Console.log(SSAPI_Plugin.getINSTANCE().getLogger(), ConsoleColor.LIGHT_PURPLE + "Launch of the caching of " + ConsoleColor.AQUA + this.sqlTableFile.getStringList( ".IdentifierList").size() + ConsoleColor.LIGHT_PURPLE + " identifier in " + ConsoleColor.AQUA + this.tableName);
                }
    ...
    public void registerAccount(T table) throws
        ...
        if (!hasAccounts(valueIdentifier.toString())) {
            ...
            if (this.identifierType.equalsIgnoreCase("identifier")) {
                if (this.sqlTableFile.getStringList(".IdentifierList") == null || this.sqlTableFile.getStringList(".IdentifierList").isEmpty() || this.sqlTableFile.getStringList(".IdentifierList").size() == 1 || !this.sqlTableFile.getStringList(".IdentifierList").contains(valueIdentifier.toString())) {
                    this.sqlTableFile.set(".IdentifierList", ConvertList.list(valueIdentifier.toString()));
                } else if (!this.sqlTableFile.getStringList(".IdentifierList").contains(valueIdentifier.toString())) {
                    this.sqlTableFile.set(".IdentifierList", this.sqlTableFile.getStringList(".IdentifierList").add(valueIdentifier.toString()));
                }
    ...
    public void updateAccount(T table) throws SQLException, NoSuchMethodException, InvocationTargetException, IllegalAccessException {
        ...
        if (hasAccounts(valueIdentifier.toString())) {
            ...
            if (TableIdentifier.class.isAssignableFrom(this.table)) {
                if (this.sqlTableFile.getStringList(".TableName") == null || this.sqlTableFile.getStringList(".TableName").isEmpty() || this.sqlTableFile.getStringList(".TableName").size() == 1 && !this.sqlTableFile.getStringList(".TableName").contains(this.tableName)) {
                    this.sqlTableFile.set(".TableName", this.tableName);
                } else if (!this.sqlTableFile.getStringList(".TableName").contains(this.tableName)) {
                    List<String> tableNameList = this.sqlTableFile.getStringList(".TableName");
                    tableNameList.add(this.tableName);
                    this.sqlTableFile.set(".TableName", tableNameList);
                }
                if (this.sqlTableFile.getStringList(this.tableName + ".IdentifierList") == null || this.sqlTableFile.getStringList(this.tableName + ".IdentifierList").isEmpty() || this.sqlTableFile.getStringList(this.tableName + ".IdentifierList").size() == 1 || !this.sqlTableFile.getStringList(this.tableName + ".IdentifierList").contains(valueIdentifier.toString())) {
                    this.sqlTableFile.set(this.tableName + ".IdentifierList", ConvertList.list(valueIdentifier.toString()));
                } else if (!this.sqlTableFile.getStringList(this.tableName + ".IdentifierList").contains(valueIdentifier.toString())) {
                    this.sqlTableFile.set(this.tableName + ".IdentifierList", this.sqlTableFile.getStringList(this.tableName + ".IdentifierList").add(valueIdentifier.toString()));
                }
            }
        }
    }
    ...
    public void deleteAccounts(Object identifier) throws SQLException {
        ...
        sqlTableFile.set("." + this.tableName + ".IdentifierList", sqlTableFile.getStringList("." + this.tableName + ".IdentifierList").remove((String) identifier));

        if (TablePlayer.class.isAssignableFrom(table)) {
            statement.setString(1, identifier.toString());
            hasAccountList.remove(identifier.toString());
            sqlTableFile.set("." + this.tableName + ".IdentifierList", sqlTableFile.getStringList("." + this.tableName + ".IdentifierList").remove(identifier.toString()));
        }
    ...
    public String onPlaceholderRequest(String identifierType, @NotNull String params) {
        return someExpansion.onRequest(null, "$" + identifierType + "$" + params);
    }
}